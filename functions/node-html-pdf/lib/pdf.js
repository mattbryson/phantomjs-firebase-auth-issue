...
PDF.prototype.exec = function PdfExec (callback) {
  ...
  function onData (buffer) {
    ...
    var responsesStr = buffer.toString().trim();
    var responses = responsesStr.split('\n');
    ...
    for(var i=0; i<responses.length; i++) {
      ...
      try {
        ...
        console.error('html-pdf: Failed to parse JSON response from phantom script', `${buffer}`);
      }
  
      if(result.log) {
        console.log('html-pdf: ', result.data);
      }
  
      if(result.error) {
        console.error('html-pdf: ', result.data);
      }
  ...
  function respond (code, err, data) {
    ...
    if (code === null) {
      kill(child, onData, onError)
        ...
        else if (!err) err = new Error("html-pdf: No Data was received from Phantom.js. Exit code '" + code + "'")
    ...
    //Mb : always call callback so user can handle errors
    callback(err, data)
  }

  child.stdout.on('data', onData);
  child.stderr.on('data', onError);
  child.on('error', function onError (err) { console.log('child error (not stdErr)', err); respond(null, err) })
